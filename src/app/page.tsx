 ...
 *
 * Role-aware: investigators see only their assigned data, billing sees financial data,
 * management sees everything except financials.
 */
...
export default async function DashboardPage() {
  const [session, ctx] = await Promise.all([getServerSession(), getRoleContext()]);
  const role = session?.role ?? "client";
  const showFinancials = canViewFinancials(role);
  const isAdminOrMgmt = ["owner", "admin", "management"].includes(role);

  const data = await getDashboardData(ctx).catch(() => null);
  ...
  return (
    <div>
      ...
      {/* Quick Actions */}
      {(isAdminOrMgmt || role === "investigator") && (
      ...
      {role !== "billing" && (
        <section className="mt-8">
          <h2 className="mb-4 text-lg font-semibold text-text-primary">
            {role === "investigator" ? "My Cases" : "Priority Cases"}
          </h2>
          {cases.length === 0 ? (
            <EmptyState
              title="No cases yet"
              description={role === "investigator" ? "No cases have been assigned to you." : "Create your first case to get started."}
            />
          ) : (
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-2">
        {role !== "billing" && role !== "client" && (
          <div className="rounded-xl border border-border bg-surface p-5">
            <h2 className="mb-4 text-lg font-semibold text-text-primary">
              {role === "investigator" ? "My Tasks" : "Upcoming Tasks"}
            </h2>
            {tasks.length === 0 ? (
